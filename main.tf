resource "aws_eks_cluster" "main" {
  name = "${var.project}-${var.environment}" // name of cluster
  ...
  depends_on = [ aws_iam_role_policy_attachment.cluster_policy ]  //  one policy
  // for cluster who must attach permissions to role that's why we need to specify the policy attachment not the Role resouce block
  // not  aws_iam_role.cluster  resource block, we should attach permission also
...
resource "aws_launch_template" "node" {
  ...
  name = "${local.resource_name}-${each.key}-LT"    // launct template name for each node group
...
resource "aws_eks_node_group" "main" {
 ...
 # get Ec2 instance deatails from user
 for_each = local.active_node_groups
